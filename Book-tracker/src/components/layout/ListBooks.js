...
function ListBooks({ book }) {
  ...
  useEffect(() => {
    book.forEach((singleBook) => { //? Braking the book array into singleBooks objects, doing that we can...
      fetch(
        ...
        .then((data) => { //data provided by the API
          if (data.docs && data.docs.length > 0) { //if the data docs exists...
            const coverId = data.docs[0].cover_i; // Allocate it`s value into a variable named coverId
            if (coverId) { //If coverId have some value inside
              setCovers((prevCovers) => ({ // `prevCovers` is a reference to the previous state of `covers`
          ...
          {book.map((singleBook) => (
            <li className={styles.book} key={book.id}>
              <span>
                ...
                <img
                  src={covers[singleBook.id]
                    ? `https://covers.openlibrary.org/b/id/${covers[singleBook.id]}-M.jpg`
